...
--
DROP TRIGGER IF EXISTS `tg_Delete_BloqueiaMovimentacaoFinalSemana`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `tg_Insert_BloqueiaMovimentacaoFinalSemana`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `tg_Update_BloqueiaMovimentacaoFinalSemana`;
DELIMITER $$
...
--
-- Estrutura stand-in para view `vwcargos`
-- (Veja abaixo para a visão atual)
--
DROP VIEW IF EXISTS `vwcargos`;
CREATE TABLE IF NOT EXISTS `vwcargos` (
`id` int
,`nome` varchar(200)
,`statusRegistro` int
);
...
--
-- Estrutura para view `vwcargos`
--
DROP TABLE IF EXISTS `vwcargos`;

DROP VIEW IF EXISTS `vwcargos`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vwcargos`  AS SELECT `cargo`.`id` AS `id`, `cargo`.`nome` AS `nome`, `cargo`.`statusRegistro` AS `statusRegistro` FROM `cargo` ;
