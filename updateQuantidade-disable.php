        ...
        try {
            ...
            if (isset($_POST['quantidade']) && is_array($_POST['quantidade'])) {
                ...
                foreach ($_POST['quantidade'] as $itemId => $quantidade) {
                    ...
                    $item = $db->dbSelect("SELECT quantidade_itens FROM itens WHERE id_itens = ?", 'first', [$itemId]);

                    // Verifica se a quantidade foi alterada
                    if ($item && $item->quantidade != $quantidade) {
                        // Atualiza a quantidade no banco de dados
                        $updateSuccess = $db->dbUpdate("UPDATE itens SET quantidade_itens = ?, dataMod_itens = NOW() WHERE id_itens = ?", [$quantidade, $itemId]);

                        // Verifica se a atualização foi bem-sucedida
                        if (!$updateSuccess) {
                            // Redireciona para a página de erro se a atualização falhar
                            header('Location: formMovimentacoes.php?msgError=Erro ao atualizar a quantidade do item');
                            exit();
                        }
                    }
                }
                ...
                header('Location: formMovimentacoes.php?msgSucesso=Quantidades de itens atualizada com sucesso');
                exit();
            } else {
                // Redireciona para a página de erro se não houver itens para atualizar
                header('Location: formMovimentacoes.php?msgError=Nenhuma quantidade de item fornecida para atualização');
                exit();
            }
        } catch (Exception $ex) {
